...
func RunChaincode(ccname, ccversion, channelName string) error {
	if channelName == "" {
	...
	for _, peer := range GlobalConfig.Peers {
		...
		go func(binPath, configDir, peerAds, PeerId, OrgId, Pdn string) {
			...
			err := obj.RunShow("instantiate_chaincode", BinPath(), ConfigDir(), peerAds, ordererAddress, PeerId, OrgId, GlobalConfig.Domain, channelName, ccname, ccversion, initparam, policy)
			if err != nil {
				fmt.Println(err)
			}
		}(BinPath(), ConfigDir(), peerAddress, peer.Id, peer.OrgId, GlobalConfig.Domain)
